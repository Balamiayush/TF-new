...
export default function BehindObsidian() {
  ...
  <LayoutWrapper>
    ...
    <div className="flex items-center justify-between">
      <p className="max-w-[435px] text-[32px] font-medium leading-[110%] tracking-[-0.3px] md:text-[42px]">
  ...
  <div className="w-full pl-[calc(max(0px,100vw-1376px)/2)]">
    <div
      ref={scrollContainerRef}
      className="no-scrollbar mt-[48px] flex w-full gap-6 overflow-x-auto scroll-smooth pb-4 pr-6 lg:pr-[calc((100vw-1376px)/2)]"
      style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
    >
      {CARDS_DATA.map((card) => (
        <div
          key={card.id}
          className="flex h-[480px] w-full min-w-[320px] max-w-[365px] shrink-0 flex-col justify-between rounded-xl border border-slate-100 bg-[#F3F7FC] p-8 sm:min-w-[365px]"
        >
          {/* Image Container */}
          <div className="relative h-[220px] w-full overflow-hidden rounded-lg bg-white/50">
            <Image
              src={card.image}
              alt={card.title}
              fill
          ...
          <div className="flex flex-col gap-3">
            <p className="max-w-[251px] text-[24px] font-medium leading-[120%] tracking-[-0.3px] text-slate-900">
              {card.title}
            </p>
            <p className="font-inter text-[14px] leading-[20px] text-slate-600">
              {card.description}
            </p>
          </div>
        </div>
